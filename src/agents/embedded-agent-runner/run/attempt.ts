...
// Clawku: sandboxed server-side workspace tools; async availability checks require the
// async attempt body (the sync tool-base prepare cannot await), so they are appended here.
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    // Clawku: append sandboxed python/php workspace tools (server-side). Availability is an
    // async probe, so it cannot run in the sync tool-base prepare. Mutating the existing
    // toolsRaw array keeps the reference the bootstrap + final tool set already use.
    if (toolsEnabled && effectiveWorkspace) {
      if (await isPythonAvailable()) {
        toolsRaw.push(createPythonTool({ workspaceDir: effectiveWorkspace, maxTimeoutSec: 30 }));
      }
      if (await isPhpAvailable()) {
        toolsRaw.push(createPhpTool({ workspaceDir: effectiveWorkspace, maxTimeoutSec: 60 }));
      }
    }
