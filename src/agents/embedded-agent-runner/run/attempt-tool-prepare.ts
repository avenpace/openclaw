...
// Clawku: platform-injected handler tools (present only when platform-api provides the handler).
    ...
    : (() => {
        const allTools = createOpenClawCodingTools({
          ...
          exec: {
            ...
            installedSkillCount: attempt.installedSkillCount,
          },
          sandbox: params.sandbox,
          messageProvider: resolveAttemptToolPolicyMessageProvider(attempt),
          agentAccountId: attempt.agentAccountId,
          ...
          // Clawku: remote browser control via paired extension.
          browserHandler: attempt.browserHandler,
        ...
        // Clawku: append platform-injected handler tools before host binding + allow-filter,
        // so they honor the same tool policy as core tools. Each is gated by handler presence
        // (platform-api injects the handler per-persona; absent handler = tool not added).
        if (attempt.devicesHandler) {
          allTools.push(...createDevicesTools(attempt.devicesHandler));
        }
        if (attempt.cloudStorageHandler) {
          allTools.push(...createCloudStorageTools(attempt.cloudStorageHandler));
        }
        if (attempt.hermesMemoryHandler) {
          allTools.push(...createHermesMemoryTools(attempt.hermesMemoryHandler));
        }
        if (attempt.hermesSkillsHandler) {
          allTools.push(...createHermesSkillsTools(attempt.hermesSkillsHandler));
        }
        if (attempt.instagramHandler) {
          allTools.push(...createInstagramTools(attempt.instagramHandler));
        }
        // Image resize always available: provided handler or built-in Sharp fallback.
        allTools.push(
          ...createImageResizeTools(attempt.imageResizeHandler ?? getBuiltinImageResizeHandler()),
        );
