...
// Clawku: external-channel command routing (device proxy / safe-bin gating).
...
export function createExecTool(
  ...
  return {
    ...
    execute: async (toolCallId, args, signal, onUpdate) => {
      ...
      // ── Clawku: external-channel command routing (multi-tenant exec boundary) ──
      // Commands from an external channel (WhatsApp/Telegram/etc.) may only run read-only /
      // data "safe" bins locally on the shared server. Anything else is routed to the user's
      // own paired device (execProxy), or blocked when no device and no skills exist — so an
      // external-channel persona cannot run arbitrary code on the shared multi-tenant server.
      {
        const messageProvider = defaults?.messageProvider?.trim()?.toLowerCase();
        const externalChannelProviders = new Set([
          "whatsapp",
          "telegram",
          "discord",
          "slack",
        ...
        if (externalChannelProviders.has(messageProvider ?? "")) {
          const analysis = analyzeShellCommand({ command: params.command });
          const configuredSafeBins = Array.from(safeBins ?? []).map((bin) => bin.toLowerCase());
          const safeSet = new Set([
            ...EXTERNAL_CHANNEL_SAFE_BINS.map((bin) => bin.toLowerCase()),
            ...configuredSafeBins,
          ]);
          const isSafeCommand =
            analysis.ok &&
            ...
            analysis.segments.every((segment) => {
              ...
              return execName.length > 0 && safeSet.has(execName);
            });
          if (!isSafeCommand) {
            const installedSkillCount = defaults?.installedSkillCount ?? 0;
            if (installedSkillCount === 0 && !execProxy) {
              logInfo(
                `exec: blocked external-channel command (no skills installed): ${params.command.slice(0, 100)}`,
              );
              throw new Error(
            ...
            if (!execProxy) {
              logInfo(
                `exec: blocked unsafe external-channel command (no device paired): ${params.command.slice(0, 100)}`,
              );
              throw new Error(
                "This command requires a paired device to execute. Please connect your device using the Clawku desktop app, then try again.",
              );
            }
            logInfo(`exec: routing command to user device: ${params.command.slice(0, 100)}`);
          ...
          // Safe command: confine file-operation bins to the workspace before local exec.
          ...
          if (
            ...
            logInfo(`exec: blocked workdir outside workspace: ${effectiveWorkdir}`);
            throw new Error(
              `Working directory "${effectiveWorkdir}" is outside your workspace. File operations are restricted to your workspace directory.`,
            );
          }
          for (const segment of analysis.segments) {
            ...
            if (WORKSPACE_RESTRICTED_BINS.has(execName)) {
              ...
              if (!pathValidation.ok) {
                logInfo(
                  `exec: blocked workspace escape attempt (${pathValidation.reason}): ${params.command.slice(0, 100)}`,
                );
