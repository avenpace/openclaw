...
import { assertRequiredParams, CLAUDE_PARAM_GROUPS, createOpenClawReadTool, createSandboxedEditTool, createSandboxedReadTool, createSandboxedWriteTool, normalizeToolParams, patchToolSchemaForClaudeCompatibility, wrapToolWorkspaceRootGuard, wrapToolParamNormalization, } from "./pi-tools.read.js";
...
export function createOpenClawCodingTools(options) {
    console.log('[DEBUG-TOOLS] createOpenClawCodingTools START, hasCloudStorageHandler:', !!options?.cloudStorageHandler);
    ...
    const tools = [
        ...
        ...(() => {
            if (options?.cloudStorageHandler) {
                const cloudTools = createCloudStorageTools(options.cloudStorageHandler);
                console.log(`[DEBUG-TOOLS] Cloud storage tools created: ${cloudTools.map(t => t.name).join(', ')}`);
                return cloudTools;
            }
            console.log('[DEBUG-TOOLS] No cloudStorageHandler - cloud storage tools not created');
            return [];
        })(),
    ];
    ...
    if (isExternalChannel) {
        ...
        const cloudToolsInFinal = filtered.filter(t => t.name.startsWith('cloud_'));
        console.log(`[DEBUG-TOOLS] Final tools for external channel (${messageProvider}): ${filtered.length} total, cloud tools: ${cloudToolsInFinal.map(t => t.name).join(', ') || 'NONE'}`);
        return filtered;
    }
    const cloudToolsInFinal = withAbort.filter(t => t.name.startsWith('cloud_'));
    console.log(`[DEBUG-TOOLS] Final tools: ${withAbort.length} total, cloud tools: ${cloudToolsInFinal.map(t => t.name).join(', ') || 'NONE'}`);
    return withAbort;
}
