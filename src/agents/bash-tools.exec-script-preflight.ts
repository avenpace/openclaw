...
export async function validateScriptFileForShellBleed(params: {
  ...
  for (const relOrAbsPath of target.relOrAbsPaths) {
    ...
    // Clawku: skip for PHP — `$VAR` and superglobals like `$_GET`/`$_POST` are valid PHP syntax,
    // not shell-variable leakage, so this check would false-positive on every PHP web script.
    const envVarRegex = /\$[A-Z_][A-Z0-9_]{1,}/g;
    const first = target.kind === "php" ? null : envVarRegex.exec(content)
    if (first) {
