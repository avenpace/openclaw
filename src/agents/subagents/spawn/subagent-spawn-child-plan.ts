...
// Clawku: sanitize a subagent's requested cwd to prevent path traversal outside the
// workspace. Rejects absolute paths, `..` segments, control characters, and any char
// outside a safe subpath set; returns undefined (no cwd override) when invalid.
...
export async function resolveSubagentChildPlan(params: {
  ...
  // Clawku: sanitize the requested cwd (reject absolute/traversal paths) before resolving,
  // so a subagent cannot escape the workspace via a crafted cwd.
  const requestedCwd = sanitizeCwdPath(normalizeOptionalString(params.request.cwd));
