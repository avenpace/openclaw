...
function buildPolicyMatcher(pattern: string): RegExp {
...
function matchesPolicy(command: string, patterns?: string[]): boolean {
...
function isCommandAllowed(command: string, allowList?: string[], denyList?: string[]): boolean {
...
export type ExecToolDefaults = {
  ...
  cwd?: string;
  // Skills installed in workspace - if empty, block exec for external channels
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      if (isExternalChannel) {
        ...
        if (installedSkillCount === 0) {
          console.log(
            `[Security] Blocked exec for external channel - no skills installed: ${command.slice(0, 100)}`,
          );
          throw new Error(
      ...
      if (!isSafeCommand) {
        ...
        if (execProxy) {
          ...
          const allowedByPolicy = isCommandAllowed(
            command,
            execProxy.allowList,
            execProxy.denyList,
          );
          ...
          const envPreview = (() => {
            const paramsEnv = (args as { env?: Record<string, string> }).env;
            if (!paramsEnv) {
              return undefined;
            }
          ...
          console.log(
            `[Security] Routing command to user device (policy=${allowedByPolicy ? "allowed" : "needs-approval"}): ${command.slice(0, 100)}`,
          );
            ...
            content: [
              {
                type: "text",
                text:
                  `Command sent to your device for approval (job ${job.jobId}). ` +
          ...
          console.log(
            `[Security] Blocked unsafe command - no paired device: ${command.slice(0, 100)}`,
          );
          throw new Error(
            "This command requires execution on your paired device, but no device is connected. " +
