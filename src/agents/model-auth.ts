...
export async function resolveApiKeyForProvider(params: {
  ...
  if (params.credentialPrecedence === "env-first") {
    ...
    if (!skipEnvFallback) {
      ...
      if (envResolved) {
        const resolvedMode: ResolvedProviderAuth["mode"] = envResolved.source.includes(
          "OAUTH_TOKEN",
        )
          ? "oauth"
          : "api-key";
  ...
  // Can be set via param or config (auth.skipEnvFallback)
  const skipEnvFallback = params.skipEnvFallback ?? cfg?.auth?.skipEnvFallback ?? false;
  const envResolved = skipEnvFallback
    ? undefined
    : resolveConfigAwareEnvApiKey(cfg, provider, params.workspaceDir);
  if (envResolved) {
    const resolvedMode: ResolvedProviderAuth["mode"] = envResolved.source.includes("OAUTH_TOKEN")
      ? "oauth"
      : "api-key";
