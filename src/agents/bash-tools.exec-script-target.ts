...
function extractInterpreterScriptTargetFromArgv(
  ...
  // Clawku: PHP scripts (website-builder) get the same workspace-path confinement as
  // python/node. The shell-injection content check is skipped for PHP downstream because
  // `$var`/`$_GET` are valid PHP, not shell-variable leakage.
  if (executable === "php") {
    const script = args.find((arg) => /^[^\s]+\.php$/i.test(arg));
    return script ? { kind: "php", relOrAbsPaths: [script] } : null;
  }
  return null;
}
