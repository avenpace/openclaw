...
import { addAllowlistEntry, evaluateShellAllowlist, maxAsk, minSecurity, requiresExecApproval, resolveSafeBins, recordAllowlistUse, resolveExecApprovals, resolveExecApprovalsFromFile, buildSafeShellCommand, buildSafeBinsShellCommand, EXTERNAL_CHANNEL_SAFE_BINS, analyzeShellCommand, } from "../infra/exec-approvals.js";
...
function buildPolicyMatcher(pattern) {
...
function matchesPolicy(command, patterns) {
    if (!patterns || patterns.length === 0) {
        return false;
    }
    const normalized = command.trim();
    return patterns.some((pattern) => buildPolicyMatcher(pattern).test(normalized));
}
function isCommandAllowed(command, allowList, denyList) {
    if (denyList && denyList.length > 0 && matchesPolicy(command, denyList)) {
        return false;
    }
    if (allowList && allowList.length > 0) {
        return matchesPolicy(command, allowList);
    }
    return matchesPolicy(command, defaultPolicyAllowList);
}
export function createExecTool(defaults) {
    ...
    return {
        ...
        execute: async (_toolCallId, args, signal, onUpdate) => {
            ...
            const requestedCwd = args.workdir || defaults?.cwd;
            ...
            const isSafeCommand = analysis.ok &&
            ...
            if (isExternalChannel) {
                ...
                if (installedSkillCount === 0) {
                    console.log(`[Security] Blocked exec for external channel - no skills installed: ${command.slice(0, 100)}`);
                    throw new Error("I don't have any skills installed that would allow me to run this command. Please install the appropriate skill from ClawHub.");
                }
            }
            // SECURITY DECISION: Safe commands run locally, unsafe commands go to user's device
            if (!isSafeCommand) {
                // Command is NOT in safe whitelist - must route to user's paired device
                if (execProxy) {
                    // Check device policy (allowList/denyList) - if explicitly allowed, still route to device for approval
                    const allowedByPolicy = isCommandAllowed(command, execProxy.allowList, execProxy.denyList);
                    // Route ALL non-safe commands to device, regardless of policy
                    // Policy just determines if device will auto-approve or require manual approval
                    const envPreview = (() => {
                        const paramsEnv = args.env;
                        if (!paramsEnv) {
                            return undefined;
                        }
                        return Object.fromEntries(Object.keys(paramsEnv).map((key) => [key, "REDACTED"]));
                    })();
                    console.log(`[Security] Routing command to user device (policy=${allowedByPolicy ? "allowed" : "needs-approval"}): ${command.slice(0, 100)}`);
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Command sent to your device for approval (job ${job.jobId}). ` +
                        ...
                        details: {
                            ...
                            approvalSlug: job.jobId.slice(0, 8),
                    ...
                    // No paired device available - block unsafe command execution
                    console.log(`[Security] Blocked unsafe command - no paired device: ${command.slice(0, 100)}`);
                    throw new Error("This command requires execution on your paired device, but no device is connected. " +
                        "Please pair a device using the Clawku Client app to run this command.");
                }
            }
            // Command is safe (in EXTERNAL_CHANNEL_SAFE_BINS) - allow local execution for skills
            console.log(`[Security] Allowed safe command for local execution: ${command.slice(0, 100)}`);
            const params = args;
            if (!params.command) {
