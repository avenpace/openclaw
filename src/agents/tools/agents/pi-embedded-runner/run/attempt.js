...
export async function runEmbeddedAttempt(params) {
    ...
    try {
        ...
        // NOTE: 'read' is allowed for external channels because:
        // - When sandbox is enabled, the sandboxed read is already restricted
        // - When sandbox is NOT enabled, we wrap read with workspace root guard below
        // This allows skills to work while preventing arbitrary file reads.
        // NOTE: 'exec' is allowed but internally restricted to safe commands (curl, wget, jq, etc.)
        // See EXTERNAL_CHANNEL_SAFE_BINS in exec-approvals-analysis.ts
        const dangerousToolsForExternalChannels = new Set([
            "process",
            "write",
            "edit",
            "ls",
        ...
        // For non-sandboxed external channels, wrap the read tool to restrict it to workspace
        // This allows skills to be read while preventing arbitrary file access
        const toolsWithRestrictedRead = isExternalMessageChannel && !sandbox?.enabled
            ? toolsRaw.map((tool) => {
                if (tool.name === "read") {
                    return wrapToolWorkspaceRootGuard(tool, effectiveWorkspace);
                }
                return tool;
            })
            : toolsRaw;
        const toolsFiltered = isExternalMessageChannel
            ? toolsWithRestrictedRead.filter((tool) => !dangerousToolsForExternalChannels.has(tool.name))
            : toolsWithRestrictedRead;
        // Create skill stub tools that return helpful errors when models incorrectly call skill names as tools
        // This handles cases where models (e.g., Gemini) ignore "skills are NOT tools" instructions
        // Filter to valid function names for Google API (must start with letter/underscore, max 64 chars)
        const isValidFunctionName = (name) => {
            if (!name || name.length > 64) {
                return false;
            }
            // Must start with letter or underscore, contain only a-zA-Z0-9_.:-
            return /^[a-zA-Z_][a-zA-Z0-9_.:-]*$/.test(name);
        };
        const skillStubTools = (skillEntries ?? [])
            .filter((entry) => entry.skill?.name && isValidFunctionName(entry.skill.name))
            .map((entry) => ({
            name: entry.skill.name,
            label: entry.skill.name,
            description: `[SKILL - NOT A TOOL] Use the read tool to load ${entry.skill.filePath} instead.`,
            parameters: { type: "object", properties: {} },
            execute: async () => ({
                content: [
                    {
                        type: "text",
                        text: `ERROR: "${entry.skill.name}" is a SKILL, not a tool. To use this skill:\n1. Call: read(file_path="${entry.skill.filePath}")\n2. Follow the instructions in the SKILL.md file\n\nDo NOT call "${entry.skill.name}" as a tool again.`,
                    },
                ],
                details: {},
            }),
        }));
        const toolsWithSkillStubs = [...toolsFiltered, ...skillStubTools];
        const tools = sanitizeToolsForGoogle({ tools: toolsWithSkillStubs, provider: params.provider });
