  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (!resolved) {
        ...
        // CLAWKU: upstream throws "Unknown session" here; platform personas run embedded
        // with transcript-only sessions absent from the metadata store, so we return a
        // basic status instead of failing the session_status tool for every persona.
