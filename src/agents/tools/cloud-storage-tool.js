// cloud-storage-tool.ts
import { Type } from "@sinclair/typebox";
import { jsonResult, readStringParam, readNumberParam } from "./common.js";
var CloudStorageInfoSchema = Type.Object({});
var CloudListFilesSchema = Type.Object({
...
var CloudUploadBase64Schema = Type.Object({
...
var CloudExtractTextSchema = Type.Object({
...
var CloudCreateDocumentSchema = Type.Object({
  type: Type.Union([
    Type.Literal("docx"),
    Type.Literal("xlsx"),
    Type.Literal("pptx"),
...
var CloudDeleteFileSchema = Type.Object({
  fileId: Type.String({ description: "The file ID to delete" })
});
var CloudUpdateFileSchema = Type.Object({
...
function createCloudListFilesTool(handler) {
  return {
...
function createCloudListFoldersTool(handler) {
  return {
...
function createCloudGetFileTool(handler) {
  return {
...
function createCloudReadFileTool(handler) {
  return {
...
function createCloudUploadTool(handler) {
  return {
    ...
    execute: async (_toolCallId, params) => {
      try {
        ...
        return jsonResult({
          ...
          message: isPublic ? `File uploaded and available at: ${result.publicUrl}` : "File uploaded to cloud storage."
        });
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        return jsonResult({ error: message });
...
function createCloudUploadBase64Tool(handler) {
  return {
...
function createCloudExtractTextTool(handler) {
  return {
...
Use cloud_get_file first if unsure about file type.`,
    ...
    execute: async (_toolCallId, params) => {
      try {
        const fileId = readStringParam(params, "fileId", { required: true });
        const maxChars = readNumberParam(params, "maxChars", { integer: true }) ?? 5e4;
...
function createCloudCreateDocumentTool(handler) {
  return {
    ...
    execute: async (_toolCallId, params) => {
      try {
        ...
        const result = await handler.createDocument({
          ...
          csv: csv ?? void 0,
          sheets: Array.isArray(params.sheets) ? params.sheets : void 0,
          slides: Array.isArray(params.slides) ? params.slides : void 0,
          folderId,
          isPublic
        });
        return jsonResult({
          ...
          size: result.sizeBytes,
          publicUrl: result.publicUrl,
          message: isPublic ? `Document created and available at: ${result.publicUrl}` : "Document created in cloud storage."
        });
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        return jsonResult({ error: message });
...
function createCloudDeleteFileTool(handler) {
  return {
...
function createCloudUpdateFileTool(handler) {
  return {
...
function createCloudCreateFolderTool(handler) {
  return {
...
function createCloudDeleteFolderTool(handler) {
  return {
...
function createCloudShareFileTool(handler) {
  return {
...
function createCloudStorageTools(handler) {
  return [
...
export {
  createCloudCreateDocumentTool,
  createCloudExtractTextTool,
  createCloudUploadBase64Tool,
  createCloudUploadTool,
  createCloudUpdateFileTool,
  createCloudStorageTools,
  createCloudStorageInfoTool,
  createCloudShareFileTool,
  createCloudReadFileTool,
  createCloudListFoldersTool,
  createCloudListFilesTool,
  createCloudGetFileTool,
  createCloudDeleteFolderTool,
  createCloudDeleteFileTool,
  createCloudCreateFolderTool
};
