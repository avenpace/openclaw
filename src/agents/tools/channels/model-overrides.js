...
import { buildChannelKeyCandidates, normalizeChannelSlug, resolveChannelEntryMatchWithFallback, } from "./channel-config.js";
...
function resolveProviderEntry(modelByChannel, channel) {
    ...
    return (modelByChannel?.[normalized] ??
        modelByChannel?.[Object.keys(modelByChannel ?? {}).find((key) => {
...
function buildChannelCandidates(params) {
    ...
    return buildChannelKeyCandidates(groupId, parentGroupId, parentGroupIdResolved, groupChannel, channelBare, channelSlug, groupSubject, subjectBare, subjectSlug);
}
export function resolveChannelModelOverride(params) {
