...
/**
 * Inject `chat_template_kwargs` into the OpenAI-compatible completions payload.
 * Used by local/vLLM-style backends that customize prompt rendering via template kwargs.
 */
export function createOpenAICompletionsChatTemplateKwargsWrapper(params: {
  baseStreamFn: StreamFn | undefined;
...
/**
 * Merge an `extra_body` object into the OpenAI-compatible completions payload.
 * Lets callers forward backend-specific fields the standard schema does not model.
 */
export function createOpenAICompletionsExtraBodyWrapper(
  baseStreamFn: StreamFn | undefined,
...
/**
 * Strip the `store` field from OpenAI-compatible completions payloads.
 * Many self-hosted/compatible backends reject the OpenAI `store` flag.
 */
