...
import { parseToolsBySenderTypedKey, } from "./types.tools.js";
...
const warnedLegacyToolsBySenderKeys = new Set();
...
function resolveChannelGroupPolicyMode(cfg, channel, accountId) {
    ...
    const accountPolicy = channelConfig.accounts?.[normalizedAccountId]?.groupPolicy ??
        channelConfig.accounts?.[Object.keys(channelConfig.accounts ?? {}).find((key) => key.toLowerCase() === normalizedAccountId.toLowerCase()) ?? ""]?.groupPolicy;
