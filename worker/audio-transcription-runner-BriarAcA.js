...
} from "./api-key-rotation-C_y8EVUw.js";
import {
  Nt as requireApiKey,
  Pt as resolveApiKeyForProvider,
  St as normalizeProviderId,
  Ui as normalizeChatType,
  _ as resolveIMessageAttachmentRoots,
  cc as isAudioFileName,
  g as mergeInboundPathRoots,
  h as isInboundPathAllowed,
  ic as detectMime,
  kt as normalizeGoogleModelId,
  m as DEFAULT_IMESSAGE_ATTACHMENT_ROOTS,
  oc as getFileExtension,
  r as loadConfig,
  uc as kindFromMime,
  xs as resolveOpenClawAgentDir,
} from "./config-Cz7eAoWC.js";
import {
  a as getDefaultMediaLocalRoots,
  n as fetchRemoteMedia,
  t as MediaFetchError,
} from "./fetch-DKMnlEva.js";
import { t as fetchWithSsrFGuard } from "./fetch-guard-B1KXbh6p.js";
import { n as fetchWithTimeout } from "./fetch-timeout-BoVs6cno.js";
import { D as ensureOpenClawModelsJson, t as describeImageWithModel } from "./image-CAQrEfw1.js";
import {
  H as runExec,
  et as resolveAgentModelFallbackValues,
  tt as resolveAgentModelPrimaryValue,
} from "./paths-Cp7C2R7V.js";
import { r as resolveProxyFetchFromEnv } from "./proxy-fetch-M9xSywjl.js";
import "node:process";
import {
  B as shouldLogVerbose,
  O as createSubsystemLogger,
  R as logVerbose,
  Z as resolvePreferredOpenClawTmpDir,
} from "./utils-DAQkyZZs.js";
//#region src/agents/model-catalog.ts
const log = createSubsystemLogger("model-catalog");
let modelCatalogPromise = null;
let hasLoggedModelCatalogError = false;
const defaultImportPiSdk = () => import("./pi-model-discovery-DKiWmRcI.js").then((n) => n.r);
...
function applySyntheticCatalogFallbacks(models) {
  const findCatalogEntry = (provider, id) =>
    models.find(
      (entry) =>
...
function normalizeConfiguredModelInput(input) {
  ...
  const normalized = input.filter(
    (item) => item === "text" || item === "image" || item === "document",
  );
...
function readConfiguredOptInProviderModels(config) {
  ...
  for (const [providerRaw, providerValue] of Object.entries(providers)) {
    ...
    for (const configuredModel of configuredModels) {
      ...
      const contextWindow =
        typeof contextWindowRaw === "number" && contextWindowRaw > 0 ? contextWindowRaw : void 0;
...
function isAttachmentRecord(value) {
  ...
  if (entry.path !== void 0 && typeof entry.path !== "string") {
    return false;
  }
  if (entry.url !== void 0 && typeof entry.url !== "string") {
    return false;
  }
  if (entry.mime !== void 0 && typeof entry.mime !== "string") {
    return false;
  }
...
async function postTranscriptionRequest(params) {
  return fetchWithTimeoutGuarded(
    ...
    params.timeoutMs,
    params.fetchFn,
    params.allowPrivateNetwork ? { ssrfPolicy: { allowPrivateNetwork: true } } : void 0,
  );
}
  ...
  const reason = attachments
    .flatMap((entry) => {
      return (Array.isArray(entry?.attempts) ? entry.attempts : [])
        .map((attempt) => (typeof attempt?.reason === "string" ? attempt.reason : void 0))
...
async function runCliEntry(params) {
  ...
  const outputDir = await fs$1.mkdtemp(
...
async function runAudioTranscription(params) {
  ...
  const cache = createMediaAttachmentCache(
    attachments,
    params.localPathRoots ? { localPathRoots: params.localPathRoots } : void 0,
  );
  try {
    return {
      transcript:
...
//#endregion
export {
  loadModelCatalog as C,
  findModelInCatalog as S,
  registerUnhandledRejectionHandler as _,
  resolveAutoImageModel as a,
  normalizeAttachments as b,
  applyTemplate as c,
  normalizeMediaUnderstandingChatType as d,
  resolveMediaUnderstandingScope as f,
  buildRandomTempFilePath as g,
  DEFAULT_IMAGE_MODELS as h,
  normalizeMediaAttachments as i,
  resolveConcurrency as l,
  CLI_OUTPUT_MAX_BUFFER as m,
  buildProviderRegistry as n,
  resolveMediaAttachmentLocalRoots as o,
  AUTO_IMAGE_KEY_PROVIDERS as p,
  createMediaAttachmentCache as r,
  runCapability as s,
  runAudioTranscription as t,
  resolveTimeoutMs as u,
  isAudioAttachment as v,
  modelSupportsVision as w,
  resolveAttachmentKind as x,
  normalizeAttachmentPath as y,
};
