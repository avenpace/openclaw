...
import {
  ...
  Ba as assertBrowserNavigationResultAllowed,
  Ea as withNoProxyForCdpUrl,
  La as BrowserTabNotFoundError,
  Oa as DEFAULT_TRACE_DIR,
  Pa as formatErrorMessage,
  Ra as assertBrowserNavigationAllowed,
  Sa as getChromeWebSocketUrl,
  Va as withBrowserNavigationPolicy,
  _a as normalizeCdpHttpBaseForJsonEndpoints,
  fa as formatAriaSnapshot,
  ga as getHeadersWithAuth,
  ha as fetchJson,
  ja as resolveStrictExistingPathsWithinRoot,
  ka as DEFAULT_UPLOAD_DIR,
  ma as appendCdpPath,
  pa as normalizeCdpWsUrl,
  va as withCdpSocket,
  wa as markPwAiLoaded,
  za as assertBrowserNavigationRedirectChainAllowed,
} from "./config-Cz7eAoWC.js";
import { Ft as writeFileFromPathWithinRoot } from "./paths-Cp7C2R7V.js";
...
async function isExtensionRelayCdpEndpoint(cdpUrl) {
  ...
  try {
    const version = await fetchJson(
      appendCdpPath(normalizeCdpHttpBaseForJsonEndpoints(normalized), "/json/version"),
      2e3,
    );
...
async function responseBodyViaPlaywright(opts) {
  ...
  const resp = await new Promise((resolve, reject) => {
    ...
    handler = (resp) => {
      ...
      if (!matchUrlPattern(pattern, resp.url?.() || "")) {
        return;
      }
      done = true;
      cleanup();
