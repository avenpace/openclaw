...
import {
  Fa as formatUncaughtError,
  Ia as readErrorName,
  Ma as collectErrorGraphCandidates,
  Na as extractErrorCode,
  Pa as formatErrorMessage,
  Qr as listChannelPlugins,
  Ss as withFileLock$1,
  Zr as getChannelPlugin,
  dc as normalizeMimeType,
  ft as resolveTelegramPreviewStreamMode,
  l as writeConfigFile,
  lc as isGifMedia,
  li as resolveTelegramAccount,
  o as readConfigFileSnapshotForWrite,
  oa as writeJsonAtomic,
  oc as getFileExtension,
  r as loadConfig,
  uc as kindFromMime,
} from "./config-Cz7eAoWC.js";
import { t as resolveTelegramFetch } from "./fetch-BWingQe0.js";
import {
  a as loadWebMedia,
  n as markdownToIR,
  s as resolveMarkdownTableMode,
  t as chunkMarkdownIR,
} from "./ir-DUSE-mNJ.js";
import { t as buildOutboundMediaLoadOptions } from "./load-options-9-C7C5wO.js";
import { d as redactSensitiveText, vt as normalizeAccountId } from "./paths-Cp7C2R7V.js";
import {
  c as resolveStateDir,
  d as resolveRequiredHomeDir,
  l as expandHomePrefix,
  o as resolveOAuthDir,
} from "./paths-eFexkPEh.js";
import { n as normalizePollInput } from "./polls-aCINrXY1.js";
import { n as makeProxyFetch } from "./proxy-fetch-M9xSywjl.js";
import { t as renderMarkdownWithMarkers } from "./render-CypHTXnD.js";
import {
  i as recordChannelActivity,
  n as createTelegramRetryRunner,
} from "./retry-policy-DtCRuljj.js";
import {
  n as normalizeTelegramLookupTarget,
  r as parseTelegramTarget,
  t as normalizeTelegramChatId,
} from "./targets-D4pwl9LR.js";
import {
  I as danger,
...
function formatLocationText(location) {
  ...
  if (resolved.source === "live" || resolved.isLive) {
    ...
    header = `📍 ${[resolved.name, resolved.address].filter(Boolean).join(" — ")} (${coords}${accuracy})`;
  } else {
...
const TELEGRAM_VOICE_AUDIO_EXTENSIONS = new Set([".oga", ".ogg", ".opus", ".mp3", ".m4a"]);
/**
...
function buildSenderLabel(msg, senderId) {
  ...
  const fallbackId =
    (senderId != null && `${senderId}`.trim() ? `${senderId}`.trim() : void 0) ??
...
function hasBotMention(msg, botUsername) {
  ...
  for (const ent of entities) {
    ...
    if (text.slice(ent.offset, ent.offset + ent.length).toLowerCase() === mention) {
      return true;
    }
  }
  return false;
...
function describeReplyTarget(msg) {
  ...
  const forwardedFrom = replyLike?.forward_origin
    ? (resolveForwardOrigin(replyLike.forward_origin) ?? void 0)
    : void 0;
  return {
...
function buildTelegramLink(link, text) {
  ...
  if (isAutoLinkedFileRef(href, text.slice(link.start, link.end))) {
    return null;
  }
...
function wrapSegmentFileRefs(text, codeDepth, preDepth, anchorDepth) {
  ...
  return text
    .replace(FILE_REFERENCE_PATTERN, wrapStandaloneFileRef)
    .replace(ORPHANED_TLD_PATTERN, (match, prefix, tld) =>
...
function renderTelegramHtmlText(text, options = {}) {
  if ((options.textMode ?? "markdown") === "html") {
    return text;
  }
...
function findTelegramHtmlSafeSplitIndex(text, maxLength) {
  ...
  if (lastAmpersand < text.lastIndexOf(";", normalizedMaxLength - 1)) {
    return normalizedMaxLength;
  }
...
function cleanupExpired(entry) {
  ...
  for (const [msgId, timestamp] of entry.timestamps) {
    if (now - timestamp > TTL_MS) entry.timestamps.delete(msgId);
  }
}
/**
...
function resolveTelegramClientOptions(account) {
  ...
  const fetchImpl = resolveTelegramFetch(proxyUrl ? makeProxyFetch(proxyUrl) : void 0, {
...
function buildTelegramThreadReplyParams(params) {
  ...
  const threadIdParams = buildTelegramThreadParams(
...
//#endregion
export {
  addChannelAllowFromStoreEntry as $,
  buildTypingThreadParams as A,
  resolveTelegramMediaPlaceholder as B,
  buildGroupLabel as C,
  buildTelegramGroupPeerId as D,
  buildTelegramGroupFrom as E,
  hasBotMention as F,
  normalizeAllowFrom as G,
  resolveTelegramStreamMode as H,
  normalizeForwardedContext as I,
  isVoiceCompatibleAudio as J,
  normalizeDmAllowFromWithStore as K,
  resolveTelegramDirectPeerId as L,
  expandTextLinks as M,
  extractTelegramLocation as N,
  buildTelegramParentPeer as O,
  getTelegramTextParts as P,
  resolveGroupAllowFromSources as Q,
  resolveTelegramForumThreadId as R,
  resolveCronStorePath as S,
  buildSenderName as T,
  resolveTelegramThreadSpec as U,
  resolveTelegramReplyId as V,
  isSenderAllowed as W,
  isSenderIdAllowed as X,
  firstDefined as Y,
  mergeDmAllowFromSources as Z,
  renderTelegramHtmlText as _,
  editMessageTelegram as a,
  getPairingAdapter as at,
  withTelegramApiErrorLogging as b,
  sendPollTelegram as c,
  toLocationContext as ct,
  resolveTelegramVoiceSend as d,
  readChannelAllowFromStore as et,
  wasSentByBot as f,
  markdownToTelegramHtml as g,
  markdownToTelegramChunks as h,
  editMessageReplyMarkupTelegram as i,
  writeJsonFileAtomically as it,
  describeReplyTarget as j,
  buildTelegramThreadParams as k,
  sendStickerTelegram as l,
  isSafeToRetrySendError as m,
  createForumTopicTelegram as n,
  upsertChannelPairingRequest as nt,
  reactMessageTelegram as o,
  listPairingChannels as ot,
  isRecoverableTelegramNetworkError as p,
  resolveSenderAllowMatch as q,
  deleteMessageTelegram as r,
  readJsonFileWithFallback as rt,
  sendMessageTelegram as s,
  formatLocationText as st,
  buildInlineKeyboard as t,
  removeChannelAllowFromStoreEntry as tt,
  sendTypingTelegram as u,
  wrapFileReferencesInHtml as v,
  buildSenderLabel as w,
  loadCronStore as x,
  splitTelegramCaption as y,
  resolveTelegramGroupAllowFromContext as z,
};
