import { P as jidToE164, Z as info, nt as success, r as defaultRuntime, st as getChildLogger, z as resolveUserPath } from "./subsystem-CZjf--pX.js";
import { o as resolveOAuthDir } from "./paths-DCNrSyZW.js";
import { r as formatCliCommand } from "./env-Cm5puq-Z.js";
import { f as DEFAULT_ACCOUNT_ID, m as normalizeOptionalAccountId, p as normalizeAccountId } from "./session-key-DRugVm48.js";
...
function resolveWebCredsEncPath(authDir) {
...
function hasWebCredsSync(authDir) {
	try {
		const encStats = fs.statSync(resolveWebCredsEncPath(authDir));
		if (encStats.isFile() && encStats.size > 1) {return true;}
	} catch {}
	try {
...
async function webAuthExists(authDir = resolveDefaultWebAuthDir()) {
	...
	try {
		const encStats = await fs$1.stat(encCredsPath);
		if (encStats.isFile() && encStats.size > 1) {return true;}
	} catch {}
	try {
...
async function clearLegacyBaileysAuthState(authDir) {
	...
	const shouldDelete = (name) => {
		...
		if (name === "creds.json" || name === "creds.json.bak" || name === "creds.json.enc") {return true;}
		if (!name.endsWith(".json") && !name.endsWith(".json.enc")) {return false;}
...
function readWebSelfId(authDir = resolveDefaultWebAuthDir()) {
	...
	try {
		...
		if (!fs.existsSync(credsPath)) {
			const encCredsPath = resolveWebCredsEncPath(resolvedAuthDir);
			if (fs.existsSync(encCredsPath)) {return {
				e164: null,
				jid: null,
				encrypted: true
			};}
		...
		return {
			e164: jid ? jidToE164(jid, { authDir: resolvedAuthDir }) : null,
			jid
		};
	} catch {
		return {
...
function getWebAuthAgeMs(authDir = resolveDefaultWebAuthDir()) {
	const resolvedAuthDir = resolveUserPath(authDir);
	try {
		const encStats = fs.statSync(resolveWebCredsEncPath(resolvedAuthDir));
		return Date.now() - encStats.mtimeMs;
	} catch {}
	try {
		const stats = fs.statSync(resolveWebCredsPath(resolvedAuthDir));
...
//#endregion
export { getWebAuthAgeMs as a, maybeRestoreCredsFromBackup as c, readWebSelfId as d, resolveDefaultWebAuthDir as f, webAuthExists as h, WA_WEB_AUTH_DIR as i, pickWebChannel as l, resolveWebCredsPath as m, resolveWhatsAppAccount as n, logWebSelfId as o, resolveWebCredsBackupPath as p, createAccountListHelpers as r, logoutWeb as s, hasAnyWhatsAppAuth as t, readCredsJsonRaw as u };