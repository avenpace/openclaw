...
import {
  At as normalizeProviders,
  Gt as isNonSecretApiKeyMarker,
  Mt as getApiKeyForModel,
  Nt as requireApiKey,
  Qn as sanitizeUserFacingText,
  Ts as normalizeSecretInput,
  i as projectConfigOntoRuntimeSourceSnapshot,
  jt as resolveImplicitProviders,
  n as getRuntimeConfigSourceSnapshot,
  pt as createConfigRuntimeEnv,
  r as loadConfig,
  xs as resolveOpenClawAgentDir,
  xt as normalizeModelRef,
} from "./config-Cz7eAoWC.js";
import {
  et as resolveAgentModelFallbackValues,
  f as redactToolDetail,
  tt as resolveAgentModelPrimaryValue,
} from "./paths-Cp7C2R7V.js";
import { _ as shortenHomeInString, c as isRecord } from "./utils-DAQkyZZs.js";
...
function coerceDisplayValue(value, opts = {}) {
  ...
  if (typeof value === "number") {
    if (!Number.isFinite(value)) {
      return opts.includeNonFinite ? String(value) : void 0;
    }
...
function resolveReadDetail(args) {
  ...
  if (limit !== void 0) {
    return `first ${limit} ${limit === 1 ? "line" : "lines"} of ${path}`;
  }
  return `from ${path}`;
}
function resolveWriteDetail(toolKey, args) {
  ...
  const path =
    resolvePathArg(record) ?? (typeof record.url === "string" ? record.url.trim() : void 0);
  if (!path) {
    return;
  }
  if (toolKey === "attach") {
...
function resolveWebFetchDetail(args) {
  ...
  const maxChars =
    typeof record.maxChars === "number" && Number.isFinite(record.maxChars) && record.maxChars > 0
      ...
      : void 0;
  const suffix = [
    mode ? `mode ${mode}` : void 0,
    maxChars !== void 0 ? `max ${maxChars} chars` : void 0,
  ]
  ...
  const id = (
    (
      (params.provider === "minimax"
        ...
        : null) ?? models.find((m) => Boolean((m?.id ?? "").trim()) && m.input?.includes("image"))
    )?.id ?? ""
  ).trim();
...
function loadPiModelDiscoveryRuntime() {
  piModelDiscoveryRuntimePromise ??= import("./pi-model-discovery-runtime-QD-80_GF.js");
...
//#endregion
export {
  extractTextFromChatContent as C,
  ensureOpenClawModelsJson as D,
  minimaxUnderstandImage as E,
  isInsideCode as S,
  isMinimaxVlmProvider as T,
  stripThinkingTagsFromText as _,
  resolveProviderVisionModelFromConfig as a,
  stripReasoningTagsFromText as b,
  extractThinkingFromTaggedStream as c,
  inferToolMetaFromArgs as d,
  isAssistantMessage as f,
  stripModelSpecialTokens as g,
  stripMinimaxToolCallXml as h,
  decodeDataUrl as i,
  extractThinkingFromTaggedText as l,
  stripDowngradedToolCallText as m,
  coerceImageAssistantText as n,
  extractAssistantText as o,
  promoteThinkingTagsToBlocks as p,
  coerceImageModelConfig as r,
  extractAssistantThinking as s,
  describeImageWithModel as t,
  formatReasoningMessage as u,
  formatToolSummary as v,
  isMinimaxVlmModel as w,
  findCodeRegions as x,
  resolveToolDisplay as y,
};
