import { D as loadPluginManifestRegistry, Dn as isDangerousHostEnvVarName, F as resolveEffectiveEnableState, I as resolveMemorySlotDecision, M as MANIFEST_KEY, P as normalizePluginsConfig, cn as isPathInsideWithRealpath, j as LEGACY_MANIFEST_KEYS } from "./config-5NZVrebk.js";
import { T as CONFIG_DIR, ft as resolvePreferredOpenClawTmpDir, t as createSubsystemLogger, z as resolveUserPath } from "./subsystem-CZjf--pX.js";
import { G as isPathInside, I as normalizeSkillFilter, T as resolveOpenClawPackageRootSync } from "./agent-scope-ScGrwqyV.js";
import { n as assertNoPathAliasEscape } from "./path-alias-guards-BLdY2C8K.js";
import { n as parseBooleanValue } from "./env-Cm5puq-Z.js";
...
const VERBOSE_LOGS = process.env.VERBOSE_LOGS === "true" || false;
function debugLog(skillKey, msg) {
	if (VERBOSE_LOGS) {console.log(`[Skills Eligibility] ${skillKey}: ${msg}`);}
}
...
function shouldIncludeSkill(params) {
	...
	if (osList.length > 0 && !osList.includes(resolveRuntimePlatform()) && !remotePlatforms.some((platform) => osList.includes(platform))) {
		debugLog(skillKey, `excluded: OS mismatch (requires ${osList.join(",")}, got ${resolveRuntimePlatform()})`);
		return false;
	}
	...
	if (requiredAnyBins.length > 0) {
		if (!(requiredAnyBins.some((bin) => hasBinary(bin)) || eligibility?.remote?.hasAnyBin?.(requiredAnyBins))) {
			debugLog(skillKey, `excluded: missing any of required binaries [${requiredAnyBins.join(",")}]`);
			return false;
		}
	}
	...
	if (requiredConfig.length > 0) {
		for (const configPath of requiredConfig) {if (!isConfigPathTruthy(config, configPath)) {
...
function resolveBundledSkillsDir(opts = {}) {
	...
	if (override) {
		if (override.toLowerCase() === "none" || override.toLowerCase() === "disabled") {return;}
		return override;
	}
	try {
...
function loadSkillEntries(workspaceDir, opts) {
	...
	const workspaceSkillsOnly = process.env.OPENCLAW_WORKSPACE_SKILLS_ONLY === "1" || process.env.OPENCLAW_WORKSPACE_SKILLS_ONLY?.toLowerCase() === "true";
	const managedSkillsDir = workspaceSkillsOnly ? void 0 : opts?.managedSkillsDir ?? path.join(CONFIG_DIR, "skills");
	const workspaceSkillsDir = path.resolve(workspaceDir, "skills");
	const bundledSkillsDir = workspaceSkillsOnly ? void 0 : opts?.bundledSkillsDir ?? resolveBundledSkillsDir();
	const extraDirs = (workspaceSkillsOnly ? [] : opts?.config?.skills?.load?.extraDirs ?? []).map((d) => typeof d === "string" ? d.trim() : "").filter(Boolean);
	const pluginSkillDirs = workspaceSkillsOnly ? [] : resolvePluginSkillDirs({
		workspaceDir,
		config: opts?.config
	});
	...
	const managedSkills = managedSkillsDir ? loadSkills({
		dir: managedSkillsDir,
		source: "openclaw-managed"
	}) : [];
	const personalAgentsSkillsDir = workspaceSkillsOnly ? void 0 : path.resolve(os.homedir(), ".agents", "skills");
	const personalAgentsSkills = personalAgentsSkillsDir ? loadSkills({
