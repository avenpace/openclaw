...
import {
  Ft as resolveEnvApiKey,
  Nt as requireApiKey,
  Pa as formatErrorMessage,
  Pt as resolveApiKeyForProvider,
  ks as normalizeResolvedSecretInputString,
  lr as onSessionTranscriptUpdate,
  ws as normalizeOptionalSecretInput,
  zt as retryAsync,
} from "./config-Cz7eAoWC.js";
import { t as isTruthyEnvValue } from "./env-CvOd8P-q.js";
import { t as fetchWithSsrFGuard } from "./fetch-guard-B1KXbh6p.js";
import {
  S as resolveAgentWorkspaceDir,
  o as resolveSessionTranscriptsDirForAgent,
  v as resolveAgentConfig,
  y as resolveAgentDir,
} from "./paths-Cp7C2R7V.js";
import { c as resolveStateDir } from "./paths-eFexkPEh.js";
import {
  _ as runWithConcurrency,
  a as listSessionFilesForAgent,
  c as chunkMarkdown,
  d as hashText,
  f as isMemoryPath,
  g as remapChunkLines,
  h as parseEmbedding,
  i as buildSessionEntry,
  l as cosineSimilarity,
  m as normalizeExtraMemoryPaths,
  o as sessionPathForFile,
  p as listMemoryFiles,
  r as requireNodeSqlite,
  s as buildFileEntry,
  t as extractKeywords,
  u as ensureDir,
  v as isFileMissingError,
  y as statRegularFile,
} from "./query-expansion-VmsaGYn-.js";
import {
  O as createSubsystemLogger,
  S as truncateUtf16Safe,
...
async function createEmbeddingProvider(options) {
  ...
  const createProvider = async (id) => {
    if (id === "local") {
      return { provider: await createLocalEmbeddingProvider(options) };
    }
    if (id === "ollama") {
...
var MemoryManagerSyncOps = class {
  ...
  async removeIndexFiles(basePath) {
    await Promise.all(
      ["", "-wal", "-shm"].map((suffix) => fs$1.rm(`${basePath}${suffix}`, { force: true })),
    );
  }
  ensureSchema() {
  ...
  async syncMemoryFiles(params) {
    ...
    for (const stale of staleRows) {
      ...
      if (this.fts.enabled && this.fts.available) {
        try {
          this.db
            .prepare(`DELETE FROM ${FTS_TABLE$2} WHERE path = ? AND source = ? AND model = ?`)
...
var MemoryManagerEmbeddingOps = class extends MemoryManagerSyncOps {
  ...
  loadEmbeddingCache(hashes) {
    ...
    for (let start = 0; start < unique.length; start += batchSize) {
      ...
      const rows = this.db
        .prepare(
          `SELECT hash, embedding FROM ${EMBEDDING_CACHE_TABLE$1}\n WHERE provider = ? AND model = ? AND provider_key = ? AND hash IN (${placeholders})`,
        )
  ...
  upsertEmbeddingCache(entries) {
    ...
    const stmt = this.db.prepare(
      `INSERT INTO ${EMBEDDING_CACHE_TABLE$1} (provider, model, provider_key, hash, embedding, dims, updated_at)\n VALUES (?, ?, ?, ?, ?, ?, ?)\n ON CONFLICT(provider, model, provider_key, hash) DO UPDATE SET\n   embedding=excluded.embedding,\n   dims=excluded.dims,\n   updated_at=excluded.updated_at`,
    );
  ...
  async resetBatchFailureCount() {
    await this.withBatchFailureLock(async () => {
      if (this.batchFailureCount > 0) {
        log$1.debug("memory embeddings: batch recovered; resetting failure count");
      }
  ...
  async runBatchWithFallback(params) {
    ...
    try {
      ...
      log$1.warn(
        `memory embeddings: ${params.provider} batch failed (${failure.count}/${BATCH_FAILURE_LIMIT$1}); ${suffix}; falling back to non-batch embeddings: ${message}`,
      );
      return await params.fallback();
    }
  }
  ...
  async indexFile(entry, options) {
    ...
    if (this.fts.enabled && this.fts.available) {
      try {
        this.db
          .prepare(`DELETE FROM ${FTS_TABLE$1} WHERE path = ? AND source = ? AND model = ?`)
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (vectorReady && embedding.length > 0) {
        ...
        this.db
          .prepare(`INSERT INTO ${VECTOR_TABLE$1} (id, embedding) VALUES (?, ?)`)
          .run(id, vectorToBlob$1(embedding));
      }
      if (this.fts.enabled && this.fts.available) {
        this.db
          .prepare(
...
//#endregion
export {
  closeAllMemoryIndexManagers as n,
  resolveMemorySearchConfig as r,
