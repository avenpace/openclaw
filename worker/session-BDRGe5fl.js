...
import {
  ...
  Js as resolveDefaultWebAuthDir,
  Ks as readCredsJsonRaw,
  Qs as VERSION,
  Ws as maybeRestoreCredsFromBackup,
  Xs as resolveWebCredsPath,
  Ys as resolveWebCredsBackupPath,
    ...
    state: {
      ...
      keys: {
        get: async (type, ids) => {
          ...
          await Promise.all(
            ids.map(async (id) => {
              const value = await readEncryptedJson(
                path.join(authDir, `${type}-${id}.json`),
                encryptionKey,
              );
...
async function createWaSocket(printQr, verbose, opts = {}) {
  ...
  const browserConfig = opts.codePairing ? Browsers.windows("Edge") : ["openclaw", "cli", VERSION];
  if (opts.codePairing) {
  ...
  sock.ev.on("connection.update", (update) => {
    try {
      ...
      if (connection === "close") {
        if (getStatusCode(lastDisconnect?.error) === DisconnectReason.loggedOut) {
          console.error(
            danger(
...
function getStatusCode(err) {
  ...
  if (nestedErr) {
    return nestedErr?.output?.statusCode ?? nestedErr?.status;
  }
}
