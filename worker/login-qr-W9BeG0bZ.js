...
async function waitForWebLogin(opts = {}) {
  ...
  while (true) {
    ...
    if ((await Promise.race([login.waitPromise.then(() => "done"), timeout])) === "timeout") {
      return {
        connected: false,
        message: "Still waiting for the QR scan. Let me know when you’ve scanned it.",
      };
    ...
    if (login.error) {
      ...
      if (login.errorStatus === 515) {
        if ((await restartLoginSocket(login, runtime)) && isLoginFresh(login)) {
          continue;
        }
      }
...
async function startWebLoginWithCode(opts) {
  ...
  if (hasWeb && !opts.force) {
    return {
      message: `WhatsApp is already linked (${selfId.e164 ?? selfId.jid ?? "unknown"}). Use force=true for fresh pairing.`,
    };
  }
  await resetActiveLogin(account.accountId);
  const phoneNumber = opts.phoneNumber.replace(/\s+/g, "");
  if (!/^\+\d{10,15}$/.test(phoneNumber)) {
    return { message: "Invalid phone number format. Use E.164 format: +6281234567890" };
  }
  try {
    const { rm, mkdir } = await import("node:fs/promises");
  ...
  const codePromise = new Promise((resolve, reject) => {
    const timeout = setTimeout(
      () => reject(/* @__PURE__ */ new Error("Timeout getting pairing code")),
      3e4,
    );
    let resolved = false;
    const handler = async (update) => {
...
function getCodePairingStatus(opts) {
  ...
  console.log(
    `[getCodePairingStatus] codeAge=${Math.round(codeAge / 1e3)}s, phoneNumber=${login.phoneNumber}, error=${login.error}, codeGeneratedAt=${login.codeGeneratedAt}`,
  );
  if (codeAge > CODE_EXPIRY_MS && !login.error) {
    console.log(
      `[getCodePairingStatus] Code is stale (${Math.round(codeAge / 1e3)}s old), marking as expired`,
    );
    return {
      active: false,
      pairingCode: login.pairingCode,
