...
import { r as setActiveWebListener } from "./active-listener-cGsaJEJ4.js";
import { _ as registerUnhandledRejectionHandler } from "./audio-transcription-runner-BriarAcA.js";
import "./fetch-guard-B1KXbh6p.js";
import "./api-key-rotation-C_y8EVUw.js";
import "./proxy-fetch-M9xSywjl.js";
import {
  B as resolveIdentityNamePrefix,
  C as buildMentionRegexes,
  E as enqueueSystemEvent,
  F as buildAgentSessionKey,
  I as deriveLastRoutePolicy,
  K as resolvePinnedMainDmOwnerFromAllowlist,
  L as resolveAgentRoute,
  R as resolveInboundLastRouteSessionKey,
  S as sleepWithAbort,
  T as formatDurationPrecise,
  U as readStoreAllowFromForDmPolicy,
  V as resolveMessagePrefix,
  W as resolveDmGroupAccessWithCommandGate,
  Y as resolveInboundDebounceMs,
  _ as formatInboundEnvelope,
  b as parseActivationCommand,
  c as resolveInboundSessionEnvelopeContext,
  d as createReplyPrefixOptions,
  f as buildHistoryContextFromEntries,
  h as dispatchReplyWithBufferedBlockDispatcher,
  l as createConnectedChannelStatusPatch,
  m as shouldAckReactionForWhatsApp,
  n as shouldComputeCommandAuthorized,
  p as recordPendingHistoryEntryIfEnabled,
  t as hasControlCommand,
  u as resolveMentionGating,
  v as getReplyFromConfig,
  w as normalizeMentionText,
  x as computeBackoff,
  y as normalizeGroupActivation,
...
import {
  $s as formatCliCommand,
  Bs as WA_WEB_AUTH_DIR,
  Ei as resolveWhatsAppMediaMaxBytes,
  Gs as pickWebChannel,
  Hs as logWebSelfId,
  Ri as resolveChannelGroupPolicy,
  Ti as resolveWhatsAppAccount,
  Vs as getWebAuthAgeMs,
  Wi as resolveGroupSessionKey,
  Zs as webAuthExists,
  fr as recordSessionMetaFromInbound,
  mr as updateLastRoute,
  r as loadConfig,
  ur as loadSessionStore,
  zi as resolveChannelGroupRequireMention,
...
import { i as getAgentScopedMediaLocalRoots } from "./fetch-DKMnlEva.js";
import "./manager-HEs65wLz.js";
import "./query-expansion-VmsaGYn-.js";
import {
  a as loadWebMedia,
  d as chunkMarkdownTextWithMode,
  h as resolveTextChunkLimit,
  m as resolveChunkMode,
  s as resolveMarkdownTableMode,
} from "./ir-DUSE-mNJ.js";
import { t as loginWeb } from "./login-nttlq0iZ.js";
import {
  i as markdownToWhatsApp,
  r as sendReactionWhatsApp,
  t as sendMessageWhatsApp,
} from "./outbound-DI_7oljz.js";
import {
  at as DEFAULT_MAIN_KEY,
  ct as buildGroupHistoryKey,
  dt as normalizeAgentId,
  ot as buildAgentMainSessionKey,
  s as resolveStorePath,
} from "./paths-Cp7C2R7V.js";
import { ct as toLocationContext } from "./send-Dv6I6FHr.js";
import {
...
function createProxyMessage(payload, call) {
  ...
  return {
    ...
    reply: async (text) => {
      await call("sendMessage", [replyTo, text, void 0, void 0, { accountId }]);
    },
...
function resolveMentionTargets(msg, authDir) {
  ...
  return {
    normalizedMentions: msg.mentionedJids?.length
...
function isBotMentionedFromTargets(msg, mentionCfg, targets) {
  ...
  if (targets.selfE164) {
    ...
    if (selfDigits) {
      if (bodyClean.replace(/[^\d]/g, "").includes(selfDigits)) {
        return true;
      }
      const bodyNoSpace = msg.body.replace(/[\s-]/g, "");
      if (new RegExp(`\\+?${selfDigits}`, "i").test(bodyNoSpace)) {
        return true;
      }
    }
  }
...
function resolveGroupActivationFor(params) {
  ...
  const defaultActivation = !resolveGroupRequireMentionFor(params.cfg, params.conversationId)
    ? "always"
    : "mention";
  ...
  for (const [index, mediaUrl] of mediaList.entries()) {
    ...
    try {
      ...
      if (index === 0) {
        ...
        const fallbackText = [remainingText.shift() ?? caption ?? "", warning]
          .filter(Boolean)
          .join("\n");
        if (fallbackText) {
...
async function processMessage(params) {
  ...
  const responsePrefix =
    ...
    (configuredResponsePrefix === void 0 && isSelfChat
...
function createWebOnMessageHandler(params) {
  ...
  return async (msg) => {
    ...
    } else if (!msg.senderE164 && peerId && peerId.startsWith("+")) {
...
async function monitorWebChannel(
  ...
  while (true) {
    ...
    const listener = await (
      useWorker ? monitorWebInboxWorker : (listenerFactory ?? monitorWebInbox)
    )({
      verbose,
    ...
    if (keepAlive) {
      heartbeat = setInterval(() => {
        ...
        const minutesSinceLastMessage = lastMessageAt
          ? Math.floor((Date.now() - lastMessageAt) / 6e4)
          : null;
        const logData = {
          connectionId,
          reconnectAttempts,
    ...
    if (Date.now() - startedAt > heartbeatSeconds * 1e3) {
      reconnectAttempts = 0;
    }
