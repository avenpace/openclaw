...
import {
  _i as resolveSlackAccount,
  bi as resolveSlackBotToken,
  r as loadConfig,
} from "./config-Cz7eAoWC.js";
import {
  r as withTrustedEnvProxyGuardedFetchMode,
  t as fetchWithSsrFGuard,
} from "./fetch-guard-B1KXbh6p.js";
import {
  a as loadWebMedia,
...
import { R as logVerbose } from "./utils-DAQkyZZs.js";
...
function isSlackCustomizeScopeError(err) {
  ...
  const maybeData = err;
  if (maybeData.data?.error?.toLowerCase() !== "missing_scope") {
    return false;
  }
  if (maybeData.data?.needed?.toLowerCase()?.includes("chat:write.customize")) {
    return true;
  }
  return [
...
async function resolveChannelId(client, recipient) {
  if (!(recipient.kind === "user" || /^U[A-Z0-9]+$/i.test(recipient.id))) {
...
//#endregion
export {
  resolveSlackWebClientOptions as a,
  buildSlackBlocksFallbackText as c,
  createSlackWebClient as i,
  parseSlackTarget as l,
  markdownToSlackMrkdwnChunks as n,
  parseSlackBlocksInput as o,
  normalizeSlackOutboundText as r,
  validateSlackBlocksArray as s,
  sendMessageSlack as t,
  resolveSlackChannelId as u,
};
