...
import {
  ...
  D as logWarn,
  I as danger,
  J as resolveNodeRequireFromMeta,
  O as createSubsystemLogger,
  T as logError,
  f as pathExists$1,
  h as resolveUserPath,
  w as logDebug,
...
function pathEscapeError(params) {
  return /* @__PURE__ */ new Error(
    `Path escapes ${params.boundaryLabel} (${shortPath$1(params.rootPath)}): ${shortPath$1(params.absolutePath)}`,
  );
}
function symlinkEscapeError(params) {
  return /* @__PURE__ */ new Error(
    `Symlink escapes ${params.boundaryLabel} (${shortPath$1(params.rootCanonicalPath)}): ${shortPath$1(params.symlinkPath)}`,
  );
}
...
async function readWorkspaceOnboardingState(statePath) {
  try {
    return (
      parseWorkspaceOnboardingState(await fs$1.readFile(statePath, "utf-8")) ?? {
        version: WORKSPACE_STATE_VERSION,
      }
    );
  } catch (err) {
...
//#endregion
export {
  normalizeStringEntriesLower as $,
  DEFAULT_BOOTSTRAP_FILENAME as A,
  copyFileWithinRoot as At,
  resolveOpenClawPackageRootSync as B,
  isNotFoundPathError as Bt,
  resolveDefaultAgentId as C,
  isAcpSessionKey as Ct,
  resolveSessionAgentIds as D,
  resolveThreadParentSessionKey as Dt,
  resolveSessionAgentId as E,
  parseAgentSessionKey as Et,
  DEFAULT_USER_FILENAME as F,
  writeFileFromPathWithinRoot as Ft,
  canUseBoundaryFileOpen as G,
  runExec as H,
  normalizeWindowsPathForComparison as Ht,
  ensureAgentWorkspace as I,
  writeFileWithinRoot as It,
  openVerifiedFileSync as J,
  openBoundaryFile as K,
  filterBootstrapFilesForSession as L,
  PATH_ALIAS_POLICIES as Lt,
  DEFAULT_IDENTITY_FILENAME as M,
  openFileWithinRoot as Mt,
  DEFAULT_SOUL_FILENAME as N,
  readFileWithinRoot as Nt,
  DEFAULT_AGENTS_FILENAME as O,
  SafeOpenError as Ot,
  DEFAULT_TOOLS_FILENAME as P,
  readLocalFileSafely as Pt,
  normalizeStringEntries as Q,
  loadWorkspaceBootstrapFiles as R,
  assertNoPathAliasEscape as Rt,
  resolveAgentWorkspaceDir as S,
  getSubagentDepth as St,
  resolveRunModelFallbacksOverride as T,
  isSubagentSessionKey as Tt,
  resolveWindowsCommandShim as U,
  runCommandWithTimeout as V,
  isPathInside as Vt,
  spawnWithFallback as W,
  normalizeAtHashSlug as X,
  normalizeSkillFilter as Y,
  normalizeHyphenSlug as Z,
  listAgentIds as _,
  DEFAULT_ACCOUNT_ID as _t,
  resolveSessionTranscriptPathInDir as a,
  DEFAULT_MAIN_KEY as at,
  resolveAgentEffectiveModelPrimary as b,
  isBlockedObjectKey as bt,
  materializeWindowsSpawnProgram as c,
  buildGroupHistoryKey as ct,
  redactSensitiveText as d,
  normalizeAgentId as dt,
  resolveAgentModelFallbackValues as et,
  redactToolDetail as f,
  normalizeMainKey as ft,
  hasConfiguredModelFallbacks as g,
  scopedHeartbeatWakeOptions as gt,
  runTasksWithConcurrency as h,
  sanitizeAgentId as ht,
  resolveSessionTranscriptPath as i,
  DEFAULT_AGENT_ID as it,
  DEFAULT_HEARTBEAT_FILENAME as j,
  createRootScopedReadFile as jt,
  DEFAULT_AGENT_WORKSPACE_DIR as k,
  appendFileWithinRoot as kt,
  resolveWindowsSpawnProgram as l,
  classifySessionKeyShape as lt,
  testRegexWithBoundedInput as m,
  resolveThreadSessionKeys as mt,
  resolveSessionFilePath as n,
  toAgentModelListLike as nt,
  resolveSessionTranscriptsDirForAgent as o,
  buildAgentMainSessionKey as ot,
  compileSafeRegex as p,
  resolveAgentIdFromSessionKey as pt,
  openBoundaryFileSync as q,
  resolveSessionFilePathOptions as r,
  markOpenClawExecEnv as rt,
  resolveStorePath as s,
  buildAgentPeerSessionKey as st,
  resolveDefaultSessionStorePath as t,
  resolveAgentModelPrimaryValue as tt,
  getDefaultRedactPatterns as u,
  isValidAgentId as ut,
  resolveAgentConfig as v,
  normalizeAccountId as vt,
  resolveEffectiveModelFallbacks as w,
  isCronSessionKey as wt,
  resolveAgentSkillsFilter as x,
  deriveSessionChatType as xt,
  resolveAgentDir as y,
  normalizeOptionalAccountId as yt,
  resolveOpenClawPackageRoot as z,
  resolvePathViaExistingAncestorSync as zt,
};
