...
import {
  Fi as resolveDiscordAccount,
  Li as normalizeDiscordToken,
  Pi as mergeDiscordAccountConfig,
  Rt as resolveRetryConfig,
  ac as extensionForMime,
  pc as maxBytesForKind,
  r as loadConfig,
  zt as retryAsync,
} from "./config-Cz7eAoWC.js";
import { t as resolveFetch } from "./fetch-CwZxIBIg.js";
import {
  a as loadWebMedia,
  d as chunkMarkdownTextWithMode,
  m as resolveChunkMode,
  o as loadWebMediaRaw,
  s as resolveMarkdownTableMode,
} from "./ir-DUSE-mNJ.js";
import { t as buildOutboundMediaLoadOptions } from "./load-options-9-C7C5wO.js";
import { vt as normalizeAccountId } from "./paths-Cp7C2R7V.js";
import { n as normalizePollInput, t as normalizePollDurationHours } from "./polls-aCINrXY1.js";
import {
  i as recordChannelActivity,
  t as createDiscordRetryRunner,
} from "./retry-policy-DtCRuljj.js";
import { t as convertMarkdownTables } from "./tables-D0-JLGfw.js";
import {
  i as requireTargetKind,
  r as parseMentionPrefixOrAtUserTarget,
  t as buildMessagingTarget,
} from "./targets-C81dAn9s.js";
import { Z as resolvePreferredOpenClawTmpDir } from "./utils-DAQkyZZs.js";
...
function formatDiscordApiErrorText(text) {
  ...
  if (!payload) {
    return trimmed.startsWith("{") && trimmed.endsWith("}") ? "unknown error" : trimmed;
  }
  const message =
...
function resolveDiscordShouldRequireMention(params) {
  ...
  if (params.isAutoThreadOwnedByBot ?? isDiscordAutoThreadOwnedByBot(params)) {
    return false;
  }
...
function chunkDiscordTextWithMode(text, opts) {
  if ((opts.chunkMode ?? "length") !== "newline") {
...
async function buildDiscordSendError(err, ctx) {
  ...
  return new DiscordSendError(
    `${missing.length ? `missing permissions in channel ${ctx.channelId}: ${missing.join(", ")}` : `missing permissions in channel ${ctx.channelId}`}. bot might be muted or blocked by role/channel overrides`,
    {
...
async function resolveChannelId(rest, recipient, request) {
  ...
  const dmChannel = await request(
    () => rest.post(Routes.userChannels(), { body: { recipient_id: recipient.id } }),
    "dm-channel",
  );
  if (!dmChannel?.id) {
...
async function createScheduledEventDiscord(guildId, payload, opts = {}) {
  return await resolveDiscordRest(opts).post(Routes.guildScheduledEvents(guildId), {
    body: payload,
  });
}
...
async function kickMemberDiscord(payload, opts = {}) {
  await resolveDiscordRest(opts).delete(Routes.guildMember(payload.guildId, payload.userId), {
    headers: payload.reason ? { "X-Audit-Log-Reason": encodeURIComponent(payload.reason) } : void 0,
  });
  return { ok: true };
}
...
async function editMessageDiscord(channelId, messageId, payload, opts = {}) {
  return await resolveDiscordRest(opts).patch(Routes.channelMessage(channelId, messageId), {
...
async function createThreadDiscord(channelId, payload, opts = {}) {
  ...
  if (!isForumLike && payload.content?.trim()) {
    await rest.post(Routes.channelMessages(thread.id), { body: { content: payload.content } });
  }
  return thread;
}
...
async function resolveDiscordSendTarget(to, opts) {
  ...
  const { channelId } = await resolveChannelId(
    rest,
    await parseAndResolveRecipient(to, opts.accountId, cfg),
    request,
  );
  return {
    rest,
...
async function sendStickerDiscord(to, stickerIds, opts = {}) {
  ...
  const rewrittenContent = content
    ? rewriteDiscordKnownMentions(content, { accountId: opts.accountId })
    : void 0;
...
//#endregion
export {
  stripUndefinedFields as $,
  unpinMessageDiscord as A,
  resolveAllowlistProviderRuntimeGroupPolicy as At,
  listScheduledEventsDiscord as B,
  editMessageDiscord as C,
  resolveDiscordSystemLocation as Ct,
  pinMessageDiscord as D,
  evaluateGroupRouteAccessForPolicy as Dt,
  listThreadsDiscord as E,
  fetchDiscord as Et,
  fetchMemberInfoDiscord as F,
  buildChannelKeyCandidates as Ft,
  uploadStickerDiscord as G,
  timeoutMemberDiscord as H,
  fetchRoleInfoDiscord as I,
  normalizeChannelSlug as It,
  editChannelDiscord as J,
  createChannelDiscord as K,
  fetchVoiceStatusDiscord as L,
  resolveChannelEntryMatchWithFallback as Lt,
  banMemberDiscord as M,
  resolveOpenProviderRuntimeGroupPolicy as Mt,
  createScheduledEventDiscord as N,
  warnMissingProviderGroupPolicyFallbackOnce as Nt,
  readMessagesDiscord as O,
  evaluateMatchedGroupAccessForPolicy as Ot,
  fetchChannelInfoDiscord as P,
  applyChannelMatchMeta as Pt,
  sendDiscordText as Q,
  kickMemberDiscord as R,
  deleteMessageDiscord as S,
  formatDiscordUserTag as St,
  listPinsDiscord as T,
  DiscordApiError as Tt,
  listGuildEmojisDiscord as U,
  removeRoleDiscord as V,
  uploadEmojiDiscord as W,
  removeChannelPermissionDiscord as X,
  moveChannelDiscord as Y,
  setChannelPermissionDiscord as Z,
  sendStickerDiscord as _,
  resolveDiscordOwnerAllowFrom as _t,
  sendDiscordComponentMessage as a,
  parseDiscordTarget as at,
  formatMention as b,
  shouldEmitDiscordReactionNotification as bt,
  parseDiscordComponentCustomId as c,
  listDiscordDirectoryPeersLive as ct,
  parseDiscordModalCustomIdForCarbon as d,
  normalizeDiscordSlug as dt,
  fetchChannelPermissionsDiscord as et,
  readDiscordComponentSpec as f,
  resolveDiscordAllowListMatch as ft,
  sendPollDiscord as g,
  resolveDiscordOwnerAccess as gt,
  sendMessageDiscord as h,
  resolveDiscordMemberAccessState as ht,
  removeReactionDiscord as i,
  createDiscordRestClient as it,
  addRoleDiscord as j,
  resolveDefaultGroupPolicy as jt,
  searchMessagesDiscord as k,
  GROUP_POLICY_BLOCKED_LABEL as kt,
  parseDiscordComponentCustomIdForCarbon as l,
  isDiscordGroupAllowedByPolicy as lt,
  resolveDiscordModalEntry as m,
  resolveDiscordGuildEntry as mt,
  reactMessageDiscord as n,
  chunkDiscordTextWithMode as nt,
  createDiscordFormModal as o,
  resolveDiscordChannelId as ot,
  resolveDiscordComponentEntry as p,
  resolveDiscordChannelConfigWithFallback as pt,
  deleteChannelDiscord as q,
  removeOwnReactionsDiscord as r,
  createDiscordClient as rt,
  formatDiscordComponentEventText as s,
  listDiscordDirectoryGroupsLive as st,
  fetchReactionsDiscord as t,
  hasAnyGuildPermissionDiscord as tt,
  parseDiscordModalCustomId as u,
  normalizeDiscordAllowList as ut,
  sendVoiceMessageDiscord as v,
  resolveDiscordShouldRequireMention as vt,
  fetchMessageDiscord as w,
  resolveTimestampMs as wt,
  createThreadDiscord as x,
  formatDiscordReactionEmoji as xt,
  sendWebhookMessageDiscord as y,
  resolveGroupDmAllow as yt,
  listGuildChannelsDiscord as z,
};
