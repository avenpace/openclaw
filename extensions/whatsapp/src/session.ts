...
export async function createWaSocket(
  ...
  opts: {
    ...
    // Clawku: multi-tenant extensions. codePairing selects the pairing-code login flow.
    // encryptionKey is accepted for the encrypted-at-rest creds feature; TODO(clawku):
    // reconnect it to an encrypted auth-state provider (upstream uses plain useMultiFileAuthState).
...
async function createWaSocketInternal(
  ...
  const makeSignalRepository = opts.onCredentialPersistenceTask
    ...
    : undefined;
  // Clawku: identify the connection as OpenClaw in WhatsApp's linked-devices list.
  const browserConfig: [string, string, string] = ["openclaw", "cli", VERSION];
